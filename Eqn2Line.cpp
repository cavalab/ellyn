...
if(ch.compare("+")==0)
	eqnstack.push_back(new n_add());
else if (ch.compare("-")==0)
	eqnstack.push_back(new n_sub());
else if(ch.compare("/")==0)
	eqnstack.push_back(new n_div());
else if (ch.compare("*")==0)
	eqnstack.push_back(new n_mul());
else if(ch.compare("sin")==0)
	eqnstack.push_back(new n_sin());
else if (ch.compare("cos")==0)
	eqnstack.push_back(new n_cos());
else if (ch.compare("exp")==0)
	eqnstack.push_back(new n_exp());
else if (ch.compare("log")==0)
	eqnstack.push_back(new n_log());
else 
	cout << "op2node failed.\n";
}
...
void Eqn2Line(string& expr,vector<shared_ptr<node>>& eqnstack)
{
	...
	for(unsigned int i =0; i<expr.size(); ++i) {
		if(tstart.empty()) 
		{
			...
			{
				while(is_letter(expr[i])){
					tstart+=expr[i];
					i++;
				}
				eqnstack.push_back(new n_sym(tstart));
			}
		...
		else 
		{
			if(isspace(expr[i])) 
			{
				...
				RPN+=tstart;
				eqnstack.push_back(new n_num(stof(tstart)));
